...
class BluetoothLE2MQTT {
    ...
    async sendCoreValues () {
        ...
        await Promise.all([
            this.handleValue(debug, 'core', 'pid', process.pid),
            this.handleValue(debug, 'core', 'version', version),
            this.handleValue(debug, 'core', 'uptime', Math.round(process.uptime())),
            this.handleValue(debug, 'core', 'watchdogTimeout', this._watchdog.timeout())
        ]);
    }
}
